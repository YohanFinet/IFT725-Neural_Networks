...
class Dense:
    ...
    def backward(self, dA, **kwargs):
        ...
        """
        # TODO
        # Récupérer les informations nécéssaires
        H = self.cache['H']
        X = self.cache['X']
        N = X.shape[0]

        # calcule le gradient de la loss par rapport à W et b
        dL_dH = self.activation['backward'](H)
        dA_dH = dA * dL_dH
        self.dW = X.T.dot(dA_dH) / N + self.reg * self.W
        self.db = np.sum(dA_dH, axis=0) / N + self.reg * self.b

        # Retourne la derivee de la couche courante par rapport à son entrée * la backProb dA
        return dA_dH.dot(self.W.T)
