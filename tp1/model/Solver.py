...
def solver(X_train, y_train, X_val, y_val, reg, optimizer, lr_decay=1.0, num_iter=100, batch_size=200, verbose=True):
    ...
    iterations_per_epoch_ceil = int(np.ceil(iterations_per_epoch))
    ...
    for it in range(num_iter):
        ...
        if (it%iterations_per_epoch_ceil == 0):
            batches = sample(X_train, y_train, batch_size)
        X_batch, y_batch = batches[it%iterations_per_epoch_ceil]
            
        # 2. Calculer la loss et les gradients
        scores = model.forward(X_batch)
        loss, dScores, softmax = model.calculate_loss(scores, y_batch, reg)
        dA = model.backward(dScores)
