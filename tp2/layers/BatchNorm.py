...
class BatchNorm:
    ...
    def forward(self, X, **kwargs):
        ...
        assert (self.num_class == X.shape[1]), f"self.num_class = {self.num_class} and X.shape[1] = {X.shape[1]}"
        if mode == "train":
            ...
            batch_norm = (X - self.test_mean) / np.sqrt(self.test_var + self.eps)
            Z = self.gamma * batch_norm + self.beta
        else:
...
class SpatialBatchNorm(BatchNorm):
    def forward(self, X, **kwargs):
        ...
        """
        N, C, H, W = X.shape
        A = super().forward(np.moveaxis(X, 1, -1).reshape(N * H * W, C), **kwargs)
    ...
    def backward(self, dA, **kwargs):
        ...
        """
        N, C, H, W = dA.shape
        dX = super().backward(np.moveaxis(dA, 1, -1).reshape(N * H * W, C), **kwargs)
