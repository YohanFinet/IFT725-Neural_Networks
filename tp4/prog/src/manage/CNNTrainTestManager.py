...
class CNNTrainTestManager(object):
    ...
    def __init__(self, model,
        ...
        self.CHECKPOINT_FOLDER_NAME = 'backup'
    ...
    def train(self, num_epochs):
        ...
        # Create variable for backup save
        best_data = {
            "best_epoch": 0,
            "current_epoch": 0,
            "best_val_acc": 0,
        ...
        for epoch in range(num_epochs):
            ...
            if self.save_train:
                # backup config
                best_data['current_epoch'] = epoch
                if self.metric_values['val_acc'][-1] > best_data['best_val_acc']:
                    best_data['best_epoch'] = epoch
                    best_data['best_val_acc'] = self.metric_values['val_acc'][-1]
                    best_data['val_loss'] = self.metric_values['val_loss'][-1]
                    best_data['train_loss'] = self.metric_values['train_loss'][-1]
                    best_data['train_acc'] = self.metric_values['train_acc'][-1]
                    self._save_current_state(best_data['best_epoch'])
                self._save_current_info(best_data)
    ...
    def accuracy(self, outputs, labels):
        ...
        """
        if isinstance(self.model, (UNet, YourUNet, YourSegNet)):
            # compute the mean of the 3 classes's dice score
            return mean_dice(outputs, labels).item()
        else:
            predicted = outputs.argmax(dim=1)
            correct = (predicted == labels).sum().item()
            return correct / (labels.size(0) * labels.size(1) * labels.size(2))
    ...
    def _save_current_state(self, best_epoch):
        current_name = self.model.__class__.__name__ + f'_model_checkpoint_{best_epoch +1}' + '.pt'
        self.model.save(f"{self.CHECKPOINT_FOLDER_NAME}\\" + current_name)
        self._remove_old_files('_model_checkpoint', current_name)

    def _save_current_info(self, best_data):
        best_epoch = best_data['best_epoch']
        current_name = self.model.__class__.__name__ + f'_infos_checkpoint_{best_epoch +1}' +'.json'
        best_data_json = json.dumps(best_data, indent=4)
        with open(f"{self.CHECKPOINT_FOLDER_NAME}\\"+ current_name, 'w+') as json_data:
            json_data.write(best_data_json)
        self._remove_old_files('_infos_checkpoint', current_name)
    
    def _remove_old_files(self, pattern, current_save_name=None):
        matching_files = self._get_macching_pattern_name(pattern)
        if current_save_name:
            matching_files.remove(current_save_name)
        for file in matching_files:
            os.remove(f"{self.CHECKPOINT_FOLDER_NAME}\\"+ file)      

    def _get_macching_pattern_name(self, pattern) -> list:
        backup_folder = os.getcwd() + f"\\{self.CHECKPOINT_FOLDER_NAME}"
        matching_files = []
